...
export const ChatMessageItem: FC<IChatMessageItemProps> = ({
  ...
  const setDownloadFile = async (fileName: string, name: string) => {
    if (fileName === "") return;
    downloadFile(fileName, Math?.random()?.toString());
  };
  ...
  const getMsgSign = (id: number, chatId: number) => {
    if (!signature && id && chatId) {
      messageSign({
        id,
        chatId,
      })?.then((res: any) => {
      ...
      >
        ...
        {item?.showAcceptButton && (
          <Tooltip
            className={`${iconCls} ${iconBorderCls}`}
            title="Принять"
            onClick={() => onCloseDiscution(item?.id as number)}
          >
            <i>
