...
const ExecutionCreate = (props: Props) => {
  ...
  const [addIncomeFile, setAddIncomeFile] = useState<UploadFileLetters[]>(
    props?.entry?.files || []
  );
  ...
  if (executors.length > 0) {
    const findActiveParent = (executor: Array<IParentApi> | IParentApi) => {
      (executor as any).childs.map((el: IParentApi) => {
        el.canAdd
          ? el.childs.map((child) => (child.canEdit = true))
          : findActiveParent(el);
      });
    };
    executors.map((el: IParentApi) =>
      el.childs.length > 0 ? findActiveParent(el) : ""
    );
  }
    ...
    <>
      <form className="tw-flex tw-flex-col tw-gap-4" onSubmit={handleSubmit}>
        ...
        <Card title="Основная информация">
          <div className="tw-py-4 tw-px-4 tw-grid tw-grid-cols-4 tw-gap-4">
            <Autocomplete
              disablePortal
              ...
              //  onChange={(_, value) => handleChangeSelect('type', value)}
