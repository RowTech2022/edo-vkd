...
export const ChatSendInput: FC<IChatSendInput> = (props) => {
  ...
  const getStart = (str: string, from: number) => {
    while (from >= 0) {
      if (str[from] === "@") {
        // Проверяем, что перед '@' или пробел, или начало строки
        return from === 0 ||
          str[from - 1] === " " ||
          str[from - 1] === "\n" ||
          str[from - 1] === "\t"
  ...
  const handleMessage = useDebounce((value: string, selectionStart: number) => {
    ...
    // Если найден символ '@' в правильной позиции (перед ним пробел или начало строки)
    if (start !== -1) {
      // Ищем индекс пробела или конца строки
      const last = value.indexOf(" ", start);

      // Если пробел не найден, используем весь остаток строки от символа '@'
      const query = value.slice(start + 1, last === -1 ? value.length : last);

      // Запрос к API с результатом
      getAvailableUsers(query).then((res: any) => {
        if (res.error) return;
