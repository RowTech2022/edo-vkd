...
import { useOpenDiscutionInIncomeMutation } from "@services/chatApiV35";
...
const IncomingCreateV3 = (props: Props) => {
  ...
  const [openDiscution] = useOpenDiscutionInIncomeMutation();
  ...
  const folderList = useFetchFolderListQuery().data?.items.map((el: any) => {
    return {
      id: el.id.toString(),
      value: el.name,
      prefix: el.prefix,
  ...
  const handleSave = (values: typeof INITIAL_VALUES) => {
    toast.promise(
      async () => {
        ...
        if ("data" in res) {
          navigate(
            `/modules/letters-v3.5/ExternalCorrespondence/corIncoming-v3.5/show/${res.data.id}`
          );
        }
      },
      {
  ...
  const closeLetter = () => {
    toast.promise(
      async () => {
        ...
        if ("data" in res) {
          navigate(`/modules/letters-v3.5/ExternalCorrespondence/`);
        }
      },
      {
  ...
  const saveUsersRow = async (tree: IParentApi) => {
    const { id } = params;
    let newTree = tree.childs?.map((el: any) => {
