...
export const ChatForm: FC<IChatForm> = ({
  ...
  const [getChatMembers, { data: members, isSuccess }] =
    useFetchExternalIncomingV3ChatMembersMutation();
  ...
  const openUsersPopup = () => {
    if (!showUsers) getChatMembers({ incommingId: incomingId });
  ...
  const setDownloadFileMain = async (fileName: string) => {
    if (fileName === "") return;
    await fileService.downloadFileV2(fileName).then((res) => {
  ...
  const btnCls =
    "tw-relative tw-text-sm tw-py-[4px] tw-px-[9px] tw-rounded-3xl tw-border-[1px] tw-border-transparent hover:tw-border-white tw-transition-colors tw-duration-300 tw-cursor-pointer tw-uppercase";

  const title = (
    <Box
      className="tw-bg-primary tw-py-4 tw-px-6"
      sx={{
        display: "flex",
    ...
    >
      <div className="tw-flex tw-items-center tw-gap-1">
        <span ref={userRef} className={btnCls} onClick={() => openUsersPopup()}>
          Участники
          {showUsers && members?.items?.length && (
            <Box
            ...
            >
              {members.items.map((user: any, idx: number) => (
                <span
                  key={idx}
        ...
        <span ref={fileRef} className={btnCls} onClick={() => openFilesPopup()}>
          Все файлы
                  ...
                  <span
                    ...
                    onClick={() => {
                      if (file.url.includes(".pdf")) {
                        setDownloadFile(file.url, file.url);
                        return;
                      }
                      setDownloadFileMain(file.url);
                    }}
                  >
                    <TextSnippetIcon fontSize="small" />
                    <i className="tw-text-[9px]">
                      {getParamFromUrl(file.url, "fileName")}
                    </i>
                  </span>
                ))}
            </Box>
