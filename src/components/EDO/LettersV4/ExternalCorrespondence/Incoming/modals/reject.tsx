...
import { IOperationInfoForUndo } from "@services/lettersApi";
import { useRejectIncomingV4Mutation } from "@services/lettersApiV4";

const Reject = ({ onClose, onSubmit, ...props }: any) => {
  const [submitRejectMutation] = useRejectIncomingV4Mutation();
  ...
  const handleReject = ({
    ...
    toast.promise(
      async () => {
        const res = await submitRejectMutation({
          id: props.entry.id,
          currentState: 1,
    ...
    <Box className="tw-absolute sm:tw-w-fit tw-w-full tw-top-1/2 tw-left-1/2 tw-transform -tw-translate-x-1/2 -tw-translate-y-1/2">
      <div className="lg:tw-w-[512px] md:tw-w-[400px] tw-w-full">
        {/* <Card title="Причина отказа"> */}
          <div className="tw-bg-[#607D8B] tw-px-8 tw-py-4 tw-rounded-t-lg">
          ...
          >
            {({ values, setFieldValue, handleChange, touched, errors }) => (
              <Form className="tw-px-8 tw-py-10 tw-bg-white md:tw-rounded-b-lg ">
                  ...
                  <Autocomplete
                    ...
                    renderInput={(params) => (
                      <TextField
                        ...
                        helperText={
                          touched.reason && typeof errors.reason === "string"
                            ? errors.reason
                            : ""
                        }
                      ></TextField>
