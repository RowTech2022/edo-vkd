...
import { InvoiceProductDTO } from "@services/invoiceApi";

interface IGoodAndServices {
  canSave?: boolean;
  total: number;
  selectedProducts: InvoiceProductDTO[];
  setSelectedProducts: (value: InvoiceProductDTO[]) => void;
...
export const GoodsAndServices: FC<IGoodAndServices> = ({
  ...
  const getCountHandler = (doc: InvoiceProductDTO) => (e: any) => {
  ...
  const getPriceHandler = (doc: InvoiceProductDTO) => (e: any) => {
    let updatedList = selectedProducts.map((el) =>
      el.id === doc.id
        ? {
            ...el,
  ...
  const getTaxpercentHandler = (doc: InvoiceProductDTO) => (e: any) => {
    let updatedList = selectedProducts.map((el) =>
      el.id === doc.id
        ? {
            ...el,
                  ...
                  <TableRow key={`doc-${doc.productId}-${doc.name}`}>
                    ...
                    <TableCell>
                      <TextField
                        ...
                        onChange={getTaxpercentHandler(doc)}
                      />
                    </TableCell>
