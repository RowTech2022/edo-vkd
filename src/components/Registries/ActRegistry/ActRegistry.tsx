...
const columns: GridColDef[] = [
  ...
  },
  {
    ...
    headerName: "Статус",
    valueFormatter: (params) => {
      return getStatusName(params.value, "act");
    },
    flex: 3,
    sortable: false,
    filterable: false,
...
const ActRegistry = () => {
  ...
  const [items, setItems] = useState<Act.ActShort[]>();
  const [filters, setFilters] = useState<Nullable<IActRequestSearch>>({
    docNo: "",
    docDate: null,
    receiver: null,
    state: null,
  ...
  const [fetchActs, { isFetching }] = useLazyFetchActQuery();

  const fetchData = async (args: Nullable<IActRequestBody>) => {
  ...
  return (
    <Card title="Реестр акт выполненных работ">
      <div className="tw-py-4">
        ...
        <DataTable
          pushUri="/modules/source-documents/act/show"
          columns={columns}
          items={items}
