...
export const LettersV35ModulePage = (props: Props) => {
  ...
  useEffect(() => {
    const edoModuleName = "letters-v3.5";
        ...
        <>
          <div className="tw-grid lg:tw-grid-cols-3 md:tw-grid-cols-2 tw-gap-6">
            {edoSubmodules.length > 0 ? (
              <>
                {edoSubmodules.filter((item: any) => item.mainModul).map((module) => (
                  <Link
                    key={module.name}
                    to={`/modules/letters-v3.5/${module.modulName}`}
                  >
                    <ModuleCard
                      title={module.name}
